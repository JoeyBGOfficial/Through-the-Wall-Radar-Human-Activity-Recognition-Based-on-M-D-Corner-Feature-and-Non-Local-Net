%% A Program for RTM Declutter Processing
...
% The code provided is specifically designed for processing the RTM.
% 
% Citation:
...
%% Main Program for RTM_Declutter
function RTM_Declutter = RTM_MTI(RTM)
    % Normalize pixel values.
    normalizedImage = mat2gray(RTM);

    % Get image size.
    [numRows, numCols] = size(normalizedImage);

    % Initialize the decluttered image.
    RTM_Declutter = zeros(numRows, numCols-1);

    % Compute horizontal differences.
    for i = 1:numRows
        for j = 1:numCols-1
            RTM_Declutter(i, j) = normalizedImage(i, j+1) - normalizedImage(i, j); % MTI process.
        end
    end
